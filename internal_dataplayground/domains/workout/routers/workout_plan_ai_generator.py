...
This file owns the generate/save flow and fuzzy exercise-name matching.
Shares the "/workout/plans" prefix with workout_plans_crud.py's router.

WO#29 Part B: exercise-history context building and the system-prompt /
prompt-template string construction that used to be inline in
generate_plan() moved to workout_plan_prompts.py (this file had grown to
375 lines) — see that module's docstring. generate_plan(),
save_generated_plan(), _fuzzy_match_exercise(), and the thin
_call_gemini_for_plan() wrapper stayed here.
...
from domains.workout.routers.workout_plan_prompts import (
    PLAN_SYSTEM_INSTRUCTION,
    _build_exercise_history_context,
    build_plan_prompt,
)
