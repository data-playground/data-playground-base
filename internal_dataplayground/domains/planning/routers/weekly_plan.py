...
NOTE (WO#30 contingency split): direct verification found this file still
at 413 lines after WO#10's own three-way split — over the 300-line router
ceiling (CONTRIBUTING.md / GOVERNANCE.md §1.2). `POST /plan/confirm` (the
longest single handler — builds day/meal/workout-session rows in a 7-day
loop) now lives in `weekly_plan_confirm.py`. All four planning routers
share the `/plan` prefix; their route paths don't overlap (verified — see
WO#30's report).
...
from domains.planning.models import (
    FitnessGoal, PlanDayStatus, PlanMealStatus,
    UserIntent,
