...
# ── BATCH CHUNKING ────────────────────────────────────────────────────────────
# Balances chunks across search categories so a handful of "big" searches
# (which return lots of cards) don't crowd out smaller/niche searches from
# ever getting an AI scoring call within max_chunks batches.
...
def score_job_batch(
    ...
    Each job dict needs job_title, description, and an identifier — either
    job_id (LinkedIn) or external_ref (Greenhouse/Lever, via job_ats_agents.py).
    Whichever is present is what comes back in the result's "ID" field, so
    callers should match results using the same key they sourced the job from.

    Returns a list of {"ID", "remote", "explanation", "fit_score",
    "qualification_analysis", "skill_gaps"} dicts — one per job Gemini
    scored. A job missing from the response should be treated by the caller
    as "not scored" (skip the insert) rather than an error; the model can
    occasionally drop a malformed entry even with explicit instructions
    not to.
    """
