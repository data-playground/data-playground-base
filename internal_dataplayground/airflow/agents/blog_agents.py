...
def _cerebras(model: str, system: str, prompt: str, temperature: float = 0.3) -> str:
    """
    OpenAI-compatible call to Cerebras Inference Cloud.
    Used by: Code Narrator, Code Commenter, Code Improver (Qwen3 235B)
             and Refiner (Llama 3.3 70B).

    Free tier: 1M tokens/day, 64K context for Qwen3 235B, no credit card.
    API is fully OpenAI-compatible — only base_url and model name differ.
    ...
    Args:
        ...
        temperature: Sampling temperature (lower = more deterministic).
                     Code tasks use 0.2-0.3; prose tasks use 0.5-0.7.
    ...
    headers = {
        "Authorization": f"Bearer {_cerebras_key()}",
        "Content-Type":  "application/json",
    }
    payload = {
        "model": model,
