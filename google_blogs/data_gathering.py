...
def fetch_and_parse_feed(start_dict, url, site_name, enrich = False):
...
def workspace_data_get(start_dict):
...
def get_google_devs_full_map(start_dict):
...
def get_ai_summary_tags(start_dict, content_list, content_type = 'article'):
...
def update_submodule_direct(start_dict):
    ...
    tree_payload = {
        ...
        "tree": [{
            ...
            "mode": "160000",
            "type": "commit",
            "sha": hashlib.sha1(generate_random_sha256().encode('utf-8')).hexdigest()
        }]
    }
    tree_res = requests.post(f"https://api.github.com/repos/{start_dict['PARENT_OWNER']}/{start_dict['PARENT_REPO']}/git/trees", headers=start_dict['HEADERS_META'], json=tree_payload).json()
    new_tree_sha = tree_res['sha']

    # 3. Create a new Commit pointing to the new Tree
    commit_payload = {
        "message": f"Update submodule {start_dict['SUBMODULE_PATH']}",
        "tree": new_tree_sha,
        "parents": [last_commit_sha]
    }
    commit_res = requests.post(f"https://api.github.com/repos/{start_dict['PARENT_OWNER']}/{start_dict['PARENT_REPO']}/git/commits", headers=start_dict['HEADERS_META'], json=commit_payload).json()
...
#### Get starting data
# blog_sites_init = init()

#### Get data for the Workspace blog
# GOOGLE_WORKSPACE_DATA = workspace_data_get(blog_sites_init)

#### Get data for various Google blogs (runs with enrich=True run the Gemini API process to get AI generated data)
# GOOGLE_APPS_DATA = fetch_and_parse_feed(blog_sites_init, GOOGLE_APPS_UPDATES, "GOOGLE_APPS_UPDATES", enrich=True)
# GOOGLE_CLOUD_DATA = fetch_and_parse_feed(blog_sites_init, GOOGLE_CLOUD_BLOG, "GOOGLE_CLOUD_BLOG")
# GOOGLE_RESEARCH_DATA = fetch_and_parse_feed(blog_sites_init, GOOGLE_RESEARCH_BLOG, "GOOGLE_RESEARCH_BLOG")
# GOOGLE_DATA = fetch_and_parse_feed(blog_sites_init, GOOGLE_BLOG, "GOOGLE_BLOG")
# GOOGLE_TECHNOLOGY_DATA = fetch_and_parse_feed(blog_sites_init, GOOGLE_TECHNOLOGY_BLOG, "GOOGLE_TECHNOLOGY_BLOG", enrich=True)
# GOOGLE_DEEPMIND_DATA = fetch_and_parse_feed(blog_sites_init, GOOGLE_DEEPMIND_BLOG, "GOOGLE_DEEPMIND_BLOG")
# GOOGLE_DEVELOPERS_DATA = fetch_and_parse_feed(blog_sites_init, GOOGLE_DEVELOPERS_BLOG, "GOOGLE_DEVELOPERS_BLOG")

#### Get full site map for google devs documentation
# # GOOGLE_DEVS_FULLMAP = get_google_devs_full_map()

#### Get data for Tableau related blogs
# flerlagetwins = fetch_and_parse_feed(blog_sites_init, "https://www.flerlagetwins.com/feeds/posts/default", "TABLEAU_flerlagetwins")
# vizwiz = fetch_and_parse_feed(blog_sites_init, "https://www.vizwiz.com/feeds/posts/default", "TABLEAU_vizwiz")
# storytellingwithdata = fetch_and_parse_feed(blog_sites_init, "https://www.storytellingwithdata.com/blog?format=rss", "TABLEAU_storytellingwithdata")
# playfairdata = fetch_and_parse_feed(blog_sites_init, "https://playfairdata.com/feed/", "TABLEAU_playfairdata")
# theinformationlab = fetch_and_parse_feed(blog_sites_init, "https://www.theinformationlab.com/", "TABLEAU_theinformationlab")

#### Get data for Google YouTube channels
# YOUTUBE_GOOGLE_WORKSPACE = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCBmwzQnSoj9b6HzNmFrg_yw", "YOUTUBE_GOOGLE_WORKSPACE")
# YOUTUBE_GOOGLE_DEVELOPERS = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UC_x5XG1OV2P6uZZ5FSM9Ttw", "YOUTUBE_GOOGLE_DEVELOPERS")
# YOUTUBE_GOOGLE_CREATORS = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCXZNlYNefXV3iMcyzszt_9Q", "YOUTUBE_GOOGLE_CREATORS")
# YOUTUBE_GOOGLE_DEEPMIND = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCP7jMXSY2xbc3KCAE0MHQ-A", "YOUTUBE_GOOGLE_DEEPMIND")
# YOUTUBE_GOOGLE = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCK8sQmJBp8GCxrOtXWBpyEA", "YOUTUBE_GOOGLE")
# YOUTUBE_GOOGLE_CLOUD = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCTMRxtyHoE3LPcrl-kT4AQQ", "YOUTUBE_GOOGLE_CLOUD")
# YOUTUBE_GOOGLE_QUANTUMAI = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCO5cgpkcYjnsdxZdEDR3Jog", "YOUTUBE_GOOGLE_QUANTUMAI")
# YOUTUBE_GOOGLE_TALKS = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCbmNph6atAoGfqLoCL_duAg", "YOUTUBE_GOOGLE_TALKS")

#### Get data for Tableau related YouTube channels
# YOUTUBE_TABLEAU_FLERLAGE = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCDyr5VgVvkmfhHpeMUB8ZDA", "YOUTUBE_TABLEAU_FLERLAGE")
# YOUTUBE_TABLEAU = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCWGrtxO6JrPSDUcgp3Qm_Gw", "YOUTUBE_TABLEAU")
# YOUTUBE_TABLEAU_TIM = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UC7HYxRWmaNlJux-X7rNLZyw", "YOUTUBE_TABLEAU_TIM")
# YOUTUBE_TABLEAU_VIZWIZ = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCTlX7UpqASrldmx5_CpG3CA", "YOUTUBE_TABLEAU_VIZWIZ")
# YOUTUBE_TABLEAU_SQLBELLE = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCW2E1sGBVde5WMMxEh5CW4w", "YOUTUBE_TABLEAU_SQLBELLE")
# YOUTUBE_TABLEAU_DATAFAM = fetch_and_parse_feed(blog_sites_init, "https://www.youtube.com/feeds/videos.xml?channel_id=UCuDUG9ZHa-IlTm6y-2Lko_Q", "YOUTUBE_TABLEAU_DATAFAM")

#### Update submodule to get latest updates to data-playground
# update_submodule_direct(blog_sites_init)
# %%
